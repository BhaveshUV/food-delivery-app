...
export let Body = () => {
    ...
    useEffect(() => {
        let fetchData = async () => {
            try {
                ...
                let restList = dataJson?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants;
                if(!restList) {
    ...
    if (!rests || rests.length === 0) {
        ...
        return (
            ...
            <div className='flex flex-col gap-4 md:gap-8 px-4 md:px-[10vw]'>
    ...
    return (
        <div className='flex flex-col gap-4 md:gap-8 px-4 md:px-[10vw]'>
            <div className="w-[100%] h-8 md:h-10 flex gap-2">
                <input type="text" className="w-[100%] rounded-lg px-1 text-lg placeholder:px-1" id="search" placeholder='Search for restaurant, cuisine or a dish' value={searchTxt} onChange={(e) => setSearchTxt(e.target.value)} />
            ...
            <div id="filter" className="flex gap-2 items-center h-6 md:h-8">
                ...
                <label htmlFor="dynamic_username" className="text-white hidden lg:inline-block">Set the user from here: </label>
                <input type="text" id="dynamic_username" className="px-2 rounded-lg h-full hidden lg:inline-block" value={username} onChange={(e) => setUser(e.target.value)}></input>
            </div>
            <div className='flex flex-wrap justify-center gap-8'>
